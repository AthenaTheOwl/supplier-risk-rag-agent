...
| R-CDCP-004 | `ops/RELEASE_LEDGER.md` with the six-commit backfill | manual review during commit; future automation may parse the ledger | `control.coordinator` |
...
| R-CDCP-007 | `.agents/skills/run-supplier-risk-query/SKILL.md` v0.1.0 | front-matter parses against `skill.schema.json`; future `validate_skills.py` lands when the second skill graduates | `product.spec-writer` (owner_role_pending_graduation: `learning.skill-curator` per `.agents/CATALOG.md` deferred catalog note) |
